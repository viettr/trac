...
#' problem with compositional constraint on the features selected by
#' sparse_log_contrast. In contrast to \code{\link{refit_sparse_log_contrast}}
#' this function refit the models based on c-lasso. Be aware that this procedure
#' does not guard against sign switches meaning that the signs of the
#' coefficients can change during the refitting procedure.
#'
#'
#' @param fit output of sparse_log_contrast
#' @param Z,y,additional_covariates same arguments as passed to
...
refit_sparse_log_contrast_classo <- function(fit, Z, y,
                                             additional_covariates = NULL,
                                             tol = 1e-5) {
  n <- nrow(Z)
  p <- ifelse(is.null(additional_covariates), ncol(Z), ncol(Z) + ncol(additional_covariates))
  nlam <- length(fit$fraclist)

  if (p != nrow(fit$beta)) {
    warning("The input dimensions (Z, y, additional_covariates) don't
            fit the dimensions of the coefficents of the fit object")
  }
  ...
  #
  X_classo <- X_classo[, -z, drop = FALSE]
  ...
  prob <- classo$classo_problem(X = X_classo,
                                C = C,
                                y = array(y))
  prob$formulation$classification <-
    ifelse(fit$method %in% c("classif", "classif_huber"), TRUE, FALSE)
  prob$formulation$concomitant <- FALSE
  if (intercept) {
    prob$formulation$intercept <- TRUE
  }
