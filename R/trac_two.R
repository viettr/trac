...
#'   that node's subtree.
#' @export
trac_two <- function(Z, y, A, additional_covariates = NULL, fraclist = NULL,
  ...
  A_bac_extended <- Matrix::cbind2(A[[1]], matrix(0, nrow = nrow(A[[1]]),
                                                  ncol = ncol(A[[2]])))
  A_euk_extended <- Matrix::cbind2(matrix(0,
                                          nrow = nrow(A[[2]]),
                                          ncol = ncol(A[[1]])),
                                   A[[2]])
  A_names <- list(c(rownames(A[[1]]), rownames(A[[2]])),
                  c(colnames(A[[1]]), colnames(A[[2]])))
  A <- Matrix::rbind2(A_bac_extended, A_euk_extended)
  dimnames(A) <- A_names
  ...
  Zbar_1 <- Matrix::rowMeans(Z[, 1:n_row_A1])
  Zbar_2 <- Matrix::rowMeans(Z[, (n_row_A1 + 1):(n_row_A1 + n_row_A2)])
  Z_clr <- Z
  Z_clr[, 1:n_row_A1] <- Z_clr[, 1:n_row_A1] - Zbar_1
  Z_clr[, (n_row_A1 + 1):(n_row_A1 + n_row_A2)] <- Z_clr[, (n_row_A1 + 1):(n_row_A1 + n_row_A2)] - Zbar_2
  ...
  for (iw in seq(num_w)) {
    ...
    C1 <- C[, 1:n_col_A1]
    C2 <- C[, (n_col_A1 + 1):(n_col_A1 + n_col_A2)]
    C1 <- c(C1, rep(0, n_col_A2))
    C2 <- c(rep(0, n_col_A1), C2)
    C <- rbind(C1, C2)
